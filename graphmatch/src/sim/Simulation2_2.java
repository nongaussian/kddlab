 ...
 * for queries
 * candidate score: number of sharing 
 */
public class Simulation2_2 extends SimulationSimRank {
	SparseIntMatrix[] neighbor_pairs;
	public Simulation2_2 (Namespace nes) {
		super(nes);
	}
	public Simulation2_2 (String[] args) {
	...
	int select_candidates(QueryNode q, CandNode[] cand) {
		...
		for (int j=0; j<dat.rnodes[q.t].size; j++) {
			if(dat.rnodes[q.t].arr[j].label<0){
				cand[idx].id = j;
				cand[idx].w = (double)neighbor_pairs[q.t].get(q.id, j);
				idx++;
			}
		}
